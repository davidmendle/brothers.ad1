const moduleDefinitions = require("../module-data.js");
...
function parseModuleDefinitions() {
  return moduleDefinitions.map((moduleDefinition) => ({ ...moduleDefinition }));
}
...
function normalizeUserRecord(uid, authUser, data = {}) {
  return {
    ...
    employerContractorId: data.employerContractorId || data.contractorId || "",
    onboardingMode: data.onboardingMode || "",
    employmentStatus: data.employmentStatus || "",
    phone: data.phone || "",
    jobTitle: data.jobTitle || "",
    onboardingAcceptedAt: data.onboardingAcceptedAt || "",
...
function filterCollectionByPermission(items, permissionSet, permissionKey, idKey = "id", explicitVisibleIds = []) {
  const config = permissionSet[permissionKey];
  const explicitVisible = new Set(
    (Array.isArray(explicitVisibleIds) ? explicitVisibleIds : [])
      .map((item) => String(item || "").trim())
      .filter(Boolean)
  );
  const applyExplicitVisibility = (list) => {
    if (!explicitVisible.size) return list;
    return list.filter((item) => {
      const ids = [item?.[idKey], item?.id, item?.key].map((value) => String(value || "").trim()).filter(Boolean);
      return ids.some((id) => explicitVisible.has(id));
    });
  };
  if (!config || config.mode === "all") {
    return applyExplicitVisibility(items.filter((item) => !config?.hidden?.includes(item[idKey])));
  }
  const allowed = new Set(config.allowed || []);
  const hidden = new Set(config.hidden || []);
  return applyExplicitVisibility(items.filter((item) => allowed.has(item[idKey]) && !hidden.has(item[idKey])));
}
