...
function getAllowedAdmins() {
  const configured = process.env.ADMIN_EMAILS || process.env.NEXT_PUBLIC_ADMIN_EMAILS || DEFAULT_ADMIN_EMAIL;
  return configured
    .split(",")
...
function getBlockedAdmins() {
  return new Set([
    ...
    ...String(process.env.BLOCKED_ADMIN_EMAILS || process.env.NEXT_PUBLIC_BLOCKED_ADMIN_EMAILS || "")
      .split(",")
      .map((email) => email.trim().toLowerCase())
      .filter(Boolean)
  ]);
