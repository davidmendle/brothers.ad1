...
const WORKSPACE_ARRAY_FIELDS = [
  ...
  "sketchRooms", "skillPacks", "dataVaults", "serviceRequests",
...
function workspaceDocumentId(companyId, field, recordId) {
  return hashSecret(`${companyId}\u0000${field}\u0000${recordId}`).slice(0, 48);
}
...
function createFirebaseRbacRouter(deps) {
  ...
  function canAccessWorkspaceRecord(session, field, record = {}, { write = false } = {}) {
    ...
    if (roleId === "franchise_owner") {
      ...
      return !recordFranchise || assignedFranchises.has(recordFranchise);
    }
  ...
  async function writeWorkspaceState(session, input = {}) {
    ...
    for (const field of WORKSPACE_ARRAY_FIELDS) {
      ...
      records.forEach((rawRecord, sortIndex) => {
        const data = cleanWorkspaceValue(rawRecord || {});
        ...
        const documentId = workspaceDocumentId(companyId, field, recordId);
    ...
    if (canAccessWorkspaceSetting(session)) {
      const existingSnapshot = await db.collection(COLLECTIONS.workspaceRecords).where("companyId", "==", companyId).get();
